...
def lc(pars, data, ch="", SuperSample = False, ss_f = 1):
    ...
    if SuperSample:
        m = batman.TransitModel(params, time, supersample_factor=ss_f, exp_time = 0.02) # used to be 5 now is 10
...
def PhaseFold(data, pars, residuals=False, binned=False, new_dict = False, SuperSample = False, ch="", ss_f = 1):
    time=data["time"]
    flux=data["flux"]
    if binned == True:
    ...
    lc_php_pars = lc(pars, php_times_pars, ch=ch, SuperSample=SuperSample, ss_f = ss_f)
...
# fix LNP function
def lnp(pars, priors, ch1, ch2, kepler, SuperSample = False, ss_f=1):
    scale = 1
    ...
    models["kepler"] = lc(pars, kepler, ch="kepler", SuperSample=SuperSample, ss_f=ss_f)
...
def run_mcmc(pars, priors, nburn, nprod, ch1, ch2, kepler, plot_corner = False, SuperSample=False, ss_f=1):
    ndim = len(pars)
    nwalkers = 2*ndim * 2
    ...
    with Pool() as pool:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, lnp,
                                        args=(priors, ch1, ch2, kepler, SuperSample, ss_f), pool=pool)
        ...
        np.savez(f"flatsample_ssf_{ss_f}", flat_sample = flat_sample, sample=sampler)  # saves flatsamples array
