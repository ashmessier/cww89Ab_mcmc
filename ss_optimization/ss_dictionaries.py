...
sig_w = math.sqrt((partial_w_partial_x * sqrtesinw[1]) ** 2 + (partial_w_partial_y * sqrtecosw[1]) ** 2)

esinw_TB = e * np.sin(w)
ecosw_TB = e * np.cos(w)

# esinw_TB error calculation
partial_m_partial_x = math.sin(w)
partial_m_partial_y = e * math.cos(w)
sig_esinw_TB = math.sqrt((partial_m_partial_x * sig_e) ** 2 + (partial_m_partial_y * sig_w) ** 2)

# ecosw_TB error calculation
partial_m_partial_x = math.cos(w)
partial_m_partial_y = -e * math.sin(w)
sig_ecosw_TB = math.sqrt((partial_m_partial_x * sig_e) ** 2 + (partial_m_partial_y * sig_w) ** 2)
...
if combo:# MCMC WORKS WITH COMBO PRIORS as PARS AND USING PRIORS_SPITZ AS T0 AND PERIOD
    ...
    combo_priors[8] = [ecosw_TB, sig_ecosw_TB]  # ecosw
    combo_priors[9] = priors_spitz[9]  # m1
    combo_priors[10] = priors_spitz[10]  # m2

# CH1
ch1 = {}
idlsav1 = np.load('../ch1_straight_test.npz')
...
# CH 2
ch2 = {}
idlsav2 = np.load('../ch2_straight_test.npz')
time2 = idlsav2['time']
...
#KEPLER
kepler = {}
data = np.loadtxt("../cww89a.Kepler.Kepler.txt")
kepler["time"] = data[:, 0]
kepler["flux"] = data[:, 1]
kepler["error"] = data[:, 2]
