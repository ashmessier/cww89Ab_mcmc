...
def run_mcmc(pars, priors, nburn, nprod, ch1, ch2, kepler, plot_corner = False, SuperSample=False, run=1):
    ...
    with Pool() as pool:
        ...
        flatsample_filepath = os.path.join(data_dir, "flat_sample_combined")
        np.savez(flatsample_filepath, flat_sample = flat_sample, sample=sampler)  # saves flatsamples array
