...
if run_mcmc_1 == False:
    mcmc_products = np.load("./Data/flat_sample_combined.npz") # uploads previous MCMC run flatsample for analysis
...
final_params_filepath = os.path.join(data_dir, "final_params")
np.savez(final_params_filepath, pars = mcmc_pars1, errors = mcmc_errs1)

ch1_filepath = os.path.join(data_dir, "ch1_data")
np.savez(ch1_filepath, time=ch1["time"], flux=ch1["mcmc_detrended_flux"], err=ch1["error"], mcmc_lc=ch1["final_mcmc_lc"],
         residuals=ch1["mcmc_detrended_flux"] - ch1["final_mcmc_lc"], x=ch1["xpos"], y=ch1["ypos"], blissflux=ch1_blissflux)

ch2_filepath = os.path.join(data_dir, "ch2_data")
np.savez(ch2_filepath, time=ch2["time"], flux=ch2["mcmc_detrended_flux"], err=ch2["error"], mcmc_lc=ch2["final_mcmc_lc"],
         residuals=ch2["mcmc_detrended_flux"] - ch2["final_mcmc_lc"], x=ch2["xpos"], y=ch2["ypos"], blissflux=ch2_blissflux)

kepler_filepath = os.path.join(data_dir, "kepler_data")
np.savez(kepler_filepath, time=kepler["time"], flux=kepler["flux"], err=kepler["error"], mcmc_lc=kepler["final_mcmc_lc"],
